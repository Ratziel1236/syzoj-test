...
app.post('/contest/:id/edit', async (req, res) => {
  try {
    ...
    if (!contest) {
      ...
      if (!['noi', 'ioi', 'acm', 'crt'].includes(req.body.type)) throw new ErrorMessage('无效的赛制。');
  ...
  try {
    ...
    if (player) {
      for (let problem of problems) {
        ...
        } else if (contest.type === 'ioi' || contest.type === 'crt') {
          if (player.score_details[problem.problem.id]) {
            let judge_state = await JudgeState.findById(player.score_details[problem.problem.id].judge_id);
            problem.status = judge_state.status;
            problem.judge_id = player.score_details[problem.problem.id].judge_id;
    ...
    if ((!contest.hide_statistics) || (contest.ended) || (isSupervisior)) {
      ...
      for (let problem of problems) {
        ...
        if (contest.type === 'ioi' || contest.type === 'noi' || contest.type === 'crt') {
        ...
        for (let player of players) {
          if (player.score_details[problem.problem.id]) {
            ...
            if ((contest.type === 'noi' || contest.type === 'ioi' || contest.type === 'crt') && player.score_details[problem.problem.id].score > 0) {
...
app.get('/contest/:id/ranklist', async (req, res) => {
  try {
    ...
    // if contest is non-public, both system administrators and contest administrators can see it.
    if (!contest.is_public && (!res.locals.user || !isSupervisior)) throw new ErrorMessage('比赛未公开，请耐心等待 (´∀ `)');
...
function getDisplayConfig(contest) {
  return {
    ...
    showUsage: contest.type === 'crt',
    showCode: contest.type === 'crt',
